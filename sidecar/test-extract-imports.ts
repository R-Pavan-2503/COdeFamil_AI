import { parseCode } from './src/parser/treeSitterAdapter';
...
`;
    const jsResult = await parseCode(jsCode, 'javascript');
...
`;
    const cjsResult = await parseCode(cjsCode, 'javascript');
...
`;
    const goResult = await parseCode(goCode, 'go');
